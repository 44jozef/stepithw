...
function findRoomsForGroup(roomsList, group) {
  roomsList.forEach((room) => {
    if (
      group.studentsCount <= room.placesCount &&
      room.facultyName === group.facultyName
    ) {
...
function sortByName(rooms) {
  for (let i = 1; i < rooms.length; i++) {
    ...
    while (
      index > 0 &&
      rooms[index - 1].name[0].toLowerCase().charCodeAt(0) >
        rooms[index].name[0].toLowerCase().charCodeAt(0)
    ) {
